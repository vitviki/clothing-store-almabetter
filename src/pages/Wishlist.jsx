  ...
  return (
    <div className="h-max flex flex-col items-center justify-center p-4">
      {wishlistItems.length === 0 ? (
        <>
          <div className="mt-[200px] mb-[20px] font-bold text-lg">YOUR WISHLIST IS EMPTY</div>
          ...
          <div className="mt-20 grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 gap-y-6">
            {wishlistItems.map(item => (
              <div key={item.id} className="relative">
                ...
                <CiCircleRemove onClick={() => handleRemove(item.id)}
                  className= " cursor-pointer absolute top-2 right-2 h-10 w-10  text-black rounded"/>
                <button onClick={addToBag} className="w-full py-2 border-2 text-[#ff3e6c] font-bold mt-3 ">MOVE TO BAG</button>
              </div>
            ))}
          </div>
        </>
